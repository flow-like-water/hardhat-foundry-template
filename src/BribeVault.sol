...
contract BribeVault is AccessControl {
    ...
    function generateBribeVaultIdentifier(address _market, bytes32 _proposal, uint256 _proposalDeadline, address _token)
        ...
        returns (bytes32 id)
    {
        id = keccak256(abi.encodePacked(_market, _proposal, _proposalDeadline, _token));
    }
    ...
    function _transferWithPermit(Common.DepositBribeParams calldata _dp) internal {
        // Get current nonce
        (,, uint48 nonce) = PERMIT2.allowance(_dp.briber, _dp.token, address(this));
        ...
        // Transfer the tokens
        PERMIT2.transferFrom(_dp.briber, address(this), _dp.amount.toUint160(), _dp.token);
    }
    ...
    function depositBribe(Common.DepositBribeParams calldata _dp) external onlyRole(DEPOSITOR_ROLE) {
        ...
        uint256 balanceBeforeTransfer = IERC20(_dp.token).balanceOf(address(this));
        ...
        if (_dp.signature.length > 0) {
            ...
            IERC20(_dp.token).safeTransferFrom(_dp.briber, address(this), _dp.amount);
        }

        // The difference between balances before and after the transfer is the actual bribe amount
        uint256 totalBribeAmount = IERC20(_dp.token).balanceOf(address(this)) - balanceBeforeTransfer;
        ...
        do {
            ...
            bribeIdentifier = generateBribeVaultIdentifier(msg.sender, _dp.proposal, deadline, _dp.token);

            Bribe storage b = bribes[bribeIdentifier];

            // Increase bribe amount and add round error to first period
            bribeAmount = (totalBribeAmount / _dp.periods) + (i == 0 ? totalBribeAmount % _dp.periods : 0);
    ...
    function transferBribes(bytes32[] calldata rewardIdentifiers) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i; i < rLen;) {
            ...
            (uint256 feeAmount, uint256 distributorAmount) = calculateTransferAmounts(rewardIdentifier);
            ...
            uint256 distributorAmountToTransfer = distributorAmount - r.distributorAmountTransferred;
            ...
            uint256 finalAmountReceived = t.balanceOf(distributor) - distributorBalance;
            r.distributorAmountReceived += finalAmountReceived;

            emit TransferBribe(rewardIdentifier, token, feeAmount, finalAmountReceived);
